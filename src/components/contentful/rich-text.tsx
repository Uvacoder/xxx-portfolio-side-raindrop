...
const CodeBlock = dynamic(
  () => import('@/components/contentful/code-block').then((mod) => mod.CodeBlock),
  {
    ssr: false,
  }
)
const DynamicIframe = dynamic(
  () => import('@/components/contentful/iframe').then((mod) => mod.Iframe),
  {
    ssr: false,
  }
)
  ...
  return {
    ...
    renderNode: {
      ...
      [BLOCKS.EMBEDDED_ASSET]: (node) => {
        ...
        const isEagerLoading = asset.contentfulMetadata?.tags?.some(
          (tag) => tag.name === 'Eager Loading'
        )
      ...
      [INLINES.EMBEDDED_ENTRY]: async (node) => {
        ...
        switch (entry.__typename) {
          case 'ContentEmbed': {
            ...
            switch (type) {
              case 'Video': {
                const YouTubeEmbed = await import('@next/third-parties/google').then(
                ...
                return (
                  <ShowInView>
                    ...
                    />
                    {title && (
                      <div className='py-2 text-center text-xs font-light text-gray-500'>
                        {title}
                      </div>
                    )}
                  </ShowInView>
          ...
          case 'Carousel': {
            const Carousel = await import('@/components/contentful/carousel').then(
              (mod) => mod.CarouselCmp
            )
